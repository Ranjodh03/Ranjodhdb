...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    do {
        if (empty($title) || empty($location) || empty($date) || empty($description) || empty($image)) {
            ...
            header("Location: /ibighit/error.php?missingInfo=true");
            exit;
        }

        // Add new employee to the database
        $sql = "INSERT INTO tour (title, location, date, description, image) " .
        ...
        $successMessage = "Tour added correctly";
...
<head>
    ...
    <style>
          body {
            background-image: url('/ibighit/txt.jpeg'); /* Specify the path to your image */
           ...
           <input type="date" class="form-control" name="date" id="date" required >
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-3 d-grid">
                    <a class="btn btn-outline-primary" href="/ibighit/index.php" role="button">Cancel</a>
                </div>
            </div>
        </form>
    </div>
