...
if ( $_SERVER['REQUEST_METHOD'] == 'GET' ) {
    //GET method: Show the data of the tour

    if ( !isset($_GET["id"]) ) {
        header("location: /ibighit/index.php");
        exit;
    }

    $id = $_GET["id"];

    //read the row of the selected tour from database table
    $sql = "SELECT * FROM tour WHERE id=$id";
    $result = $connection->query($sql);
    $row = $result->fetch_assoc();

    if (!$row) {
        // Redirect to error.php if no tour found
        header("location: /ibighit/error.php");
        exit;
    }
    ...
    //POST method: Update the data of the tour
    ...
    do {
        ...
        $sql = "UPDATE tour SET title = ?, location = ?, date = ?, description = ?, image = ? WHERE id = ?";
...
if ($stmt = $connection->prepare($sql)) {
    ...
    $successMessage = "tour updated correctly";
    header("location: /ibighit/index.php");
    exit;
} else {
...
<style>
    body {
            background-image: url('/ibighit/purpleocean.png'); /* Specify the path to your image */
...
<div class="col-sm-3 d-grid">
    <a class="b tn btn-outline-primary" href="/ibighit/index.php" role="button">Cancel</a>
</div>
</div>
</form>
</div>
